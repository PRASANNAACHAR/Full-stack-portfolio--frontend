    ...
    const getMyApps = async () => {
      try {
        const { data } = await axios.get(
          "https://full-stack-portfolio-backend-4077.onrender.com/api/v1/softwareapplication/getall",
          { withCredentials: true }
        );
        setApps(data.softwareApplications);
      } catch (error) {
        console.error("Error fetching apps:", error.response?.data?.message || error.message);
      }
    };
    getMyApps();
  }, []);
            ...
            apps.map((element) => (
              <Card className="h-fit p-7 flex flex-col justify-center items-center gap-3" key={element._id}>
                <img
                  ...
                  onError={(e) => { e.target.src = '/placeholder.svg'; }} // Fallback image if error
                />
                <p className='text-muted-foreground text-center'>{element.name}</p>
              </Card>
            ))
